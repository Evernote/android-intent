...
public final class CreateNewNoteIntentBuilder extends IntentBuilder {
    ...
    public CreateNewNoteIntentBuilder setAppVisibility(AppVisibility visibility) {
        ...
        switch (visibility) {
            case OPEN_COMPOSER:
                mArgs.remove(EvernoteIntent.EXTRA_QUICK_SEND);
                mArgs.remove(EvernoteIntent.EXTRA_FORCE_NO_UI);
                break;

            case QUICK_SEND:
                mArgs.remove(EvernoteIntent.EXTRA_FORCE_NO_UI);
                mArgs.putBoolean(EvernoteIntent.EXTRA_QUICK_SEND, true);
                break;

            case NO_UI:
                mArgs.remove(EvernoteIntent.EXTRA_QUICK_SEND);
                mArgs.putBoolean(EvernoteIntent.EXTRA_FORCE_NO_UI, true);
                break;
